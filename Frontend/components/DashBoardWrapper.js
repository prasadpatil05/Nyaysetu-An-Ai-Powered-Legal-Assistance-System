...
export default function DashBoardWrapper({ children, page }) {
    ...
    const connectLawyers = page === "connect-lawyers" ? true : false
    const myChats = page === "my-chats" ? true : false
    const kyr = page === "kyr" ? true : false
        ...
        >
            <Flex zIndex={10} width={{ base: "100%", lg: "250px" }} backgroundColor="gray.100" flexDirection={{ base: "row", lg: "column" }} justifyContent={{ base: "center", lg: "space-between" }} alignItems={"center"} padding={{ base: 2, lg: 6 }} gap={{ base: 2, lg: 4 }} minHeight={{ base: "60px", lg: "100vh" }} position={"fixed"} bottom={0}>
                <Flex flexDirection={{ base: "row", lg: "column" }} gap={{ base: 2, lg: 4 }} width={{ base: "80%", lg: "auto" }} justifyContent={"space-evenly"}>
                    ...
                    <Link href="/know-your-rights" width="100%">
                        <Button colorScheme='gray' bg={kyr && "gray.200"} padding={2} rounded={{ base: "full", lg: 4 }} alignItems={"center"} justifyContent="flex-start" width="100%">
                            <Icon boxSize={{ base: 7, lg: 5 }} as={GiGavel} />
                            <Text display={{ base: "none", lg: "block" }} marginLeft={{ base: 0, lg: 3 }}>Know Your Rights</Text>
                        </Button>
                    </Link>
                    <Link href="/connect-lawyers" width="100%">
                        <Button colorScheme='gray' bg={connectLawyers && "gray.200"} padding={2} rounded={{ base: "full", lg: 4 }} alignItems={"center"} justifyContent="flex-start" width="100%">
                            <Icon boxSize={{ base: 7, lg: 5 }} as={RiContactsBookLine} />
                            <Text display={{ base: "none", lg: "block" }} marginLeft={{ base: 0, lg: 3 }}>Connect with Lawyers</Text>
                        </Button>
                    </Link>
                    <Link href="/my-chats" width="100%">
                        <Button colorScheme='gray' bg={myChats && "gray.200"} padding={2} rounded={{ base: "full", lg: 4 }} alignItems={"center"} justifyContent="flex-start" width="100%">
                            <Icon boxSize={{ base: 7, lg: 5 }} as={GrChat} />
                            <Text display={{ base: "none", lg: "block" }} marginLeft={{ base: 0, lg: 3 }}>My Chats</Text>
                        </Button>
                    </Link>
